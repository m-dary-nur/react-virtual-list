...
const VirtuList = ({ items, wrapperClassName = "", ...props }: any) => {
  const {
    children,
    height = "100%",
    width = "100%",
  ...
  const startItem = Math.max(
    0,
    Math.floor(scrollTop / itemHeight) - itemBuffer
  );
  const visibleItemCount = Math.min(
    items.length - startItem,
    Math.ceil(clientHeight / itemHeight) + 2 * itemBuffer
  );
  ...
  return (
    <div
      className={`react-virtuallist ${wrapperClassName}`}
